...
describe('CampaignForm — câu hỏi AI (F9/F10)', () => {
  ...
  it('giữ nguyên source AiGenerated khi lưu (không gán cứng CustomHr)', () => {
    ...
    // ⚠ ĐỔI TIỀN ĐỀ CÓ CHỦ ĐÍCH (lúc gộp vòng 3): trước đây assert `source` trong PAYLOAD gửi đi.
    // Nhưng BE (F10) CỐ Ý bỏ qua `source` client gửi — nguồn gốc là sự thật do SERVER sở hữu, đúng để
    // HR không tự dán nhãn "AiGenerated" cho câu mình gõ. Payload vì thế KHÔNG chứa `source` (có test
    // riêng khoá điều đó ở campaign-form.f10.spec.ts), nên assert ở tầng payload là assert một hợp
    // đồng KHÔNG TỒN TẠI. Ý định thật của test — provenance phải sống sót qua vòng đọc→sửa→lưu để
    // badge không biến mất — nay được assert ở TẦNG FORM, đúng chỗ nó có nghĩa.
    expect([cmp.questionSource(0), cmp.questionSource(1)]).toEqual(['AiGenerated', 'CustomHr']);
  });
  ...
  it('câu HR tự thêm mới mặc định là CustomHr', () => {
    ...
    // Cùng lý do như trên: kiểm ở tầng form. Payload không mang `source` (BE tự gán CustomHr cho câu
    // không có id), nên assert payload sẽ luôn `undefined` và test thành vô nghĩa.
    expect(cmp.questionSource(cmp.questions.length - 1)).toBe('CustomHr');
  });
