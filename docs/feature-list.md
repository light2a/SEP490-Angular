...
| **Phỏng vấn THÍCH ỨNG (INT-17)**: badge kind câu AI sinh động + gợi ý "AI đã hỏi xong → nộp bài" (`interviewComplete`); refresh() kéo câu thích ứng mới | ✅ done |
...
| **Phỏng vấn THÍCH ỨNG (INT-17)**: append `nextQuestion` từ response upload (B2B không re-fetch) → dừng ở câu thích ứng; `end`→màn tổng kết; resume merge danh sách đầy đủ từ GET; badge kind | ✅ done |
