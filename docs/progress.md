...
> **Cập nhật 2026-07-18 (INT-17 — Phỏng vấn THÍCH ỨNG, branch `feat/adaptive-interview` off `docs/reconcile-2026-07-18`, đã push):** wire FE với luồng thích ứng backend (INT-17). `UploadAnswerResult` += `transcript/nextAction/nextQuestion/interviewComplete`; `QuestionResponse`/`CampaignQuestion` += `kind`; enums `QuestionKind`+`AdaptiveAction`+nhãn. **B2C** `practice-session`: `refresh()` đã kéo câu thích ứng mới (list lớn dần) → thêm badge kind + gợi ý nộp khi `interviewComplete`. **B2B** `campaign-interview`: **append `nextQuestion` TRƯỚC `advance()`** (B2B không re-fetch → nếu không sẽ nhảy màn tổng kết thay vì hỏi tiếp) + resume merge danh sách ĐẦY ĐỦ từ GET session (không mất câu thích ứng khi reload) + badge kind. **Gate:** `ng build` AOT sạch · **vitest 105 pass** (+2 test adaptive B2B: append-and-advance, interviewComplete→review). Backward-compat (field optional). ⚠ **CHƯA merge vào `master`** (deploy line) — chờ merge/rebase. Backend migration `AddAdaptiveInterviewColumns` **đã apply lên `postgres-main` server** (07-18, 51 câu backfill kind='Seed'); feature bật khi backend đặt `Adaptive:Enabled=true` (B2C) / toggle campaign (B2B) + deploy image mới có `/decide-next`.
